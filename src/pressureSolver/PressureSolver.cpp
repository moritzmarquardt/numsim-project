...
void PressureSolver::setBoundaryValues() {
    ...
    for (int j = discretization_->pJBegin(); j < discretization_->pJEnd(); j++) {
        ...
        discretization_->p(discretization_->pIEnd() , j) = discretization_->p(discretization_->pIEnd() - 1, j); // rechts
    }
    ...
    for (int i = discretization_->pIBegin(); i < discretization_->pIEnd(); i++) {
        ...
        discretization_->p(i, discretization_->pJEnd()) = discretization_->p(i, discretization_->pJEnd() - 1); // oben
    }
}

void PressureSolver::computeResidualNorm() {
    double residual_norm_squared = 0.0;
    const int N = (discretization_->pIEnd() - discretization_->pIBegin()) * (discretization_->pJEnd() - discretization_->pJBegin()); // number of cells
    for (int i = discretization_->pIBegin(); i < discretization_->pIEnd(); i++) {
        for (int j = discretization_->pJBegin(); j < discretization_->pJEnd(); j++) {
            double rhs_ij = discretization_->rhs(i,j);
            double p_xx = (discretization_->p(i+1,j) - 2.0 * discretization_->p(i,j) + discretization_->p(i-1,j)) / (discretization_->dx() * discretization_->dx());
            double p_yy = (discretization_->p(i,j+1) - 2.0 * discretization_->p(i,j) + discretization_->p(i,j-1)) / (discretization_->dy() * discretization_->dy());
            double residual_ij = rhs_ij - (p_xx + p_yy);
