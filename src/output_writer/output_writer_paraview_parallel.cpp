...
void OutputWriterParaviewParallel::writeFileWithNumber(double currentTime, int simNumber)
{
  // communicate all data to rank 0
  gatherData();
  ...
  // Assemble the filename
  std::stringstream fileName;
  fileName << "out/output_" << std::setw(4) << setfill('0') << simNumber << "." << vtkWriter_->GetDefaultFileExtension();
